...
st.markdown('<p class="rules-font">Six cards are dealt. One card is designated as the objective card. The object of the game is to combine all the remaining five cards in any order using any of basic four arithmetic functions so that the result equals the objective. All 5 cards must be used exactly once. Once you figure out the solution, you can ask for another set. Enjoy! You can read about the origins of the game Welcome to <a href="https://en.wikipedia.org/wiki/Krypto_(game)">here</a>. </p>', unsafe_allow_html=True)
